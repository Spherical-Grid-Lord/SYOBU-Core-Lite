...
## 7. 代表例・刻印（日本語）
代表的な識別子（例: 曼荼羅cot, 1bit意味の構造素, 1bit_apple_syobu_v32.png）は `docs/examples/` に置かれ、各ファイルの SHA256 ハッシュは `docs/EXAMPLES_HASHES.md` に記録されています。これらは出所確認と違反検出のための識別子です。
